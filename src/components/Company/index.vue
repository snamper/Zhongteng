...
import updateCompany from "@/views/Cart/update/UpdateCartMode";
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.$axios
        .post(company.query, { companyId: this.searchValue })
        .then((res) => {
          const data = res.data;
          if (data.errCode == 200) {
    ...
    //修改内容
    update(value) {
      this.$axios.post(company.addOrUpdate, { ...value }).then((res) => {
        const data = res.data;
        if (data.errCode == 200) {
          this.getData();
        } else {
    ...
    delete(arr) {
      this.$axios
        .post(company.delete, { carTypeIds: arr })
        .then((res) => {
          const data = res.data;
          if (data.errCode == 200) {
