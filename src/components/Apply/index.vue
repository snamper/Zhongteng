...
import updateApply from '@/views/Repair/update'
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.$axios.post(applyInfo.query, { vin: this.searchValue }).then(res => {
        const data = res.data;
        if (data.errCode == 200) {
    ...
    delete(arr) {
      ...
      this.$axios.post(applyInfo.delete, { applicationIds: arr }).then(res => {
        const data = res.data;
        if (data.errCode == 200) {
